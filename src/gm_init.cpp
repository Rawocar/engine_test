...
int init_models_menu(CApp * pApp)
{
  ...
  // Würfel
	...
	ppModel[0]->create_cube(0.6f, 128, 128, 200, 255);
	ppModel[0]->set_mdl_pos(0.0f, 0.0f, -2.0f);

	ppModel[1]->create_cube(0.6f, 128, 128, 200, 255);
	ppModel[1]->set_mdl_pos(2.0f, 0.0f, -1.0f);

	ppModel[2]->create_cube(0.6f, 128, 128, 200, 255);
	ppModel[2]->set_mdl_pos(-2.0f, 0.0f, -1.0f);

	ppModel[3]->create_square(1.3f, 130, 0, 130);
  ...
  ppModel[4]->set_mdl_pos(0.0f, -1.0f, 0.0f);
	...
	ppModel[5]->create_sphere(0.5f, 40, 128, 128, 200);
	ppModel[5]->set_mdl_pos(-1.3f, 1.7f, 0.0f);

	ppModel[6]->create_pyramid(0.5f, 0.9f, 128, 128, 200);
	ppModel[6]->set_mdl_pos(1.3f, 1.7f, 0.0f);

	ppModel[7]->create_cone(0.5f, 0.9f, 16, 128, 128, 200);
