...
int CModel_3D::load_mdl_from_obj(const char * pPath)
{
  int exit_code = ERR_EN_NO_ERROR;
	FILE * pFile = NULL;
	unsigned int number_vertexes = 0;
	const unsigned short int SIZE = 100;
	const unsigned short int SIZEF = 30;
	char pBuffer[SIZE];
	char pFloatx[SIZEF];
	char pFloaty[SIZEF];
  char pFloatz[SIZEF];
	...
	if(pFile != NULL)
	{
		// Ermittle Anzahl an Vertexes

		while(!feof(pFile))
		{
			fgets(pBuffer, SIZE, pFile);
			
			if(pBuffer[0] == 'v' && pBuffer[1] == ' ')
				{number_vertexes++;}
		}

		this->init_vertexes(number_vertexes);
		...
		// öffne Datei erneut und lade Daten
		...
		if(pFile != NULL)
		{
			while(!feof(pFile))
			{
				...
				if(pBuffer[0] == 'v' && pBuffer[1] == ' ')
				{
					...
					strncpy(pFloatz, (pBuffer + (i - l)), l);
					l = 0;					
	
					this->add_vertex_pos(atof(pFloatx), atof(pFloaty), atof(pFloatz));
					printf("x: %f, y: %f, z: %f\n", atof(pFloatx), atof(pFloaty), atof(pFloatz));
        }

				// SPÄTER ANDERE DATEN

				for(unsigned int i = 0 ; i < this->number_vertexes ; i++)
					{this->add_vertex_col(1.0f, 1.0f, 1.0f, 1.0f);}
